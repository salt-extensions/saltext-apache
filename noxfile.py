...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/apache/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/apache/*")
            ...
            #    "--include=src/saltext/apache/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/apache/config/schemas",
    )
