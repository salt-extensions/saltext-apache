...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/saltext_apache/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/saltext_apache/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/saltext_apache/*"
            )
            ...
            #    "--include=src/saltext/saltext_apache/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/saltext_apache/config/schemas",
    )
